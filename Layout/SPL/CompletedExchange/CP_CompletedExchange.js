...
; (function () {
    ...
    function ShowFaildInfo() {
        ButtonEnable();
        document.getElementById("faildDiv").innerText = strErrMsg;
    ...
    function ClearDataBase(){
        ...
        // 存款笔数 DEPCOUNTS
        // 存款金额 DEPAMOUNT
        // 转账总笔数 TFRTOTALCOUNTS
        // 转账总金额 TFRTOTALAMOUNT
        // 转账笔数 TFRCOUNTS
        // 转账金额 TFRAMOUNT
        // 现金转账笔数 NOCARDTFRCOUNTS
        // 现金转账金额 NOCARDTFRAMOUNT
        // 冲正笔数 CWCCOUNTS
        // 冲正金额 CWCAMOUNT
        top.API.Dat.SetPersistentDataSync("CWDCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("CWDAMOUNT", "STRING", DataBase);

        top.API.Dat.SetPersistentDataSync("DEPCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("DEPAMOUNT", "STRING", DataBase);
        
        top.API.Dat.SetPersistentDataSync("TFRTOTALCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("TFRTOTALAMOUNT", "STRING", DataBase);

        top.API.Dat.SetPersistentDataSync("TFRCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("TFRAMOUNT", "STRING", DataBase);

        top.API.Dat.SetPersistentDataSync("NOCARDTFRCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("NOCARDTFRAMOUNT", "STRING", DataBase);

        top.API.Dat.SetPersistentDataSync("CWCCOUNTS", "STRING", DataBase);
        top.API.Dat.SetPersistentDataSync("CWCAMOUNT", "STRING", DataBase);
    }
    ...
    function onExchangeInitiated() {
        ...
        if (nRet == 0) {
            ...
            if (PtrPaperStatus == "FULL" || PtrPaperStatus == "LOW") {
                top.API.Ptr.Print("TOTAL_ycTCR_AccountStatement", "",top.API.gPrintTimeOut);
                top.API.Ptr.Print("TOTAL_ycTCR_AccountStatementInfo", "",top.API.gPrintTimeOut);
                top.API.Ptr.Print("TOTAL_ycTCR_BillingSummary", "",top.API.gPrintTimeOut);
            }
    ...
    function onDatSetPersistentDataComplete(DataName) {
        ...
        if ('TOTALADDOK' == DataName) {
			...
			document.getElementById("SuccessedDiv").innerText = strErrMsg;
    ...
    function onDatSetPersistentDataError(DataName, ErrorCode) {
        top.API.displayMessage("onDatSetPersistentDataError");
        ButtonEnable();
		ShowFaildInfo();
    }
    ...
    function onTcpOnRecved(tmpCheck) {
        ...
        switch (Check) {
            case '00':
                Purging="平";
                top.API.Dat.SetDataSync("PURGINGRESULTS", "STRING", Purging);
