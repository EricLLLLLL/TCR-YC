...
;(function () {
    ...
    function onScrCardAccepted() {
        ...
        if (strTrack2 == "") {
            ...
            top.API.displayMessage("top.API.gLastCardNum："+top.API.gLastCardNum);
            top.API.displayMessage("strCardNo："+strCardNo);
            if(strCardNo == top.API.gLastCardNum){
                // top.API.displayMessage("strCardNo+" + strCardNo);
                top.API.gCardno = strCardNo;
                SetCardNo2();
            }else{
                top.API.displayMessage("继续交易卡号不一致");
                Files.ErrorMsg("交易失败！");
                setTimeout(function () {
                    return CallResponse("Exit");
                }, 4000);
