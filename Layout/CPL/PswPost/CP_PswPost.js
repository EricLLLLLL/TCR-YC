...
; (function () {
    ...
    var Initialize = function () {
        ...
        var nRet;
        nRet = top.API.Dat.SetDataSync("OLDPINBLOCK", "STRING", arrOldPsw);
        top.API.displayMessage("SetDataSync arrOldPsw Return:" + nRet); 
        nRet = top.API.Dat.SetDataSync("NEWPINBLOCK", "STRING", arrNewPsw);
    ...
    function TradeFailed() {
        ...
        ReInputFlag[0] = 1;
        var nRet = top.API.Dat.SetDataSync(top.API.inputpasswordtimesTag, top.API.inputpasswordtimesType, ReInputFlag);
    ...
    function JournalPrint() {
        ...
        //获取手续费
        var Poundage = "0.00";
        var objGet3= top.API.Dat.GetDataSync(top.API.poundageTag, top.API.poundageType);                    
        if (null == objGet3) {
        top.API.displayMessage("GetDataSync poundage objGet = null");
        }
        else {                        
        top.API.displayMessage("手续费： " + objGet3);
        if (objGet3[0].length > 2) {
        Poundage = objGet3[0].substr(0,(objGet3[0].length-2));
        Poundage  += ".";
        Poundage  += objGet3[0].substr((objGet3[0].length-2),2);
        }else if (objGet3[0].length == 2) {
        Poundage = "0."+ objGet3[0];
        }else if (objGet3[0].length == 1) {
        Poundage = "0.0"+ objGet3[0];
        }
        }
        top.API.gPoundage = Poundage;
        //获取存款金额
        var Amount = top.API.CashInfo.strTransAmount + ".00";
        top.API.Jnl.PrintSync("CashInBox", "JnlNum=" + JnlNum + ", TransKind=卡存款, RetCode=" + Check
        ...
        //获取取款总额
        var Amount = top.API.CashInfo.strTransAmount + ".00";
        top.API.Jnl.PrintSync("CashOutBox1", "JnlNum=" + JnlNum + ", TransKind=卡取款, RetCode=" + Check + ", ProResult=" + tmpstrErrMsg + ", AmountMoney=" + Amount + ", Poundage=" + Poundage);
        break;
        case "INQ":
        top.API.Jnl.PrintSync("CardQuery", "JnlNum=" + JnlNum + ", TransKind=卡查询, RetCode=" + Check + ", ProResult=" + tmpstrErrMsg);
        break;
        case "QRYCUSTNAME":
        top.API.Jnl.PrintSync("QueryAccInfo", "JnlNum=" + JnlNum + ", TransKind=查户名, RetCode=" + Check + ", ProResult=" + tmpstrErrMsg);
        break;
        case "CWC":
        top.API.Jnl.PrintSync("WithdrawCorrect", "JnlNum=" + JnlNum + ", TransKind=取款冲正, RetCode=" + Check + ", ProResult=" + tmpstrErrMsg);
        break;
        default:
        if (NextPageFlag == 1) {
